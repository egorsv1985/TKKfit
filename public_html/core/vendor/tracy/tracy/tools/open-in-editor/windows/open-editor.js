var settings = {
  ...
  editor:
    '"C:\\Users\\User\\AppData\\Local\\Programs\\Microsoft VS CodeCode.exe" --goto "%file%:%line%"',
...
if (!settings.editor) {
  WScript.Echo(
    'Create variable "settings.editor" in ' + WScript.ScriptFullName
  );
  WScript.Quit();
}
...
if (action === "create" && !fileSystem.FileExists(file)) {
  shell.Run(
    'cmd /c mkdir "' + fileSystem.GetParentFolderName(file) + '"',
    0,
    1
  );
